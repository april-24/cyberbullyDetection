...
Tip: split the workload across your group - e.g. each member labels a
different third of the file. Progress is saved after every comment, and the
tool skips comments that are already labeled when you reopen it, so multiple
people can work through the same source file over several sessions (just
avoid two people annotating the exact same raw file into two DIFFERENT output
files - agree on one output file, or split the input into separate CSVs first).
"""
